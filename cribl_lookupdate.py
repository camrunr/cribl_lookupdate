...
from cribl_python_api_wrapper import auth, groups, inputs, lookups, versioning
...
def parse_args():
    ...
    parser.add_argument(
        "-l", "--leader", help="Leader URL, http(s)://leader:port", required=True
    )
    parser.add_argument(
        "-g", "--group", type=str, help="Target worker group", required=True
    )
    parser.add_argument(
        "-f",
        "--filepath",
        type=str,
        help="Path to the file we want to upload",
        required=True,
    )
    parser.add_argument(
        "-a",
        "--appenddata",
        type=str,
        help="Data to append to the existing lookup",
        required=False,
    )
    parser.add_argument(
        "-u",
        "--username",
        help="API token id (cloud) or user id (self-managed)",
        required=False,
    )
    ...
    parser.add_argument(
        ...
        "--password",
        type=Password,
        help="Specify password or secret, or get prompted for it",
...
def append_row(leader, group, auth_token, lookup_id, new_data):
    url = (
        leader
        + "/m/<WG>/system/lookups".replace("<WG>", group)
        + "/"
        + lookup_id
        + "/content"
    )
    ...
    if append_resp.status_code == 200:
        ...
        print(
            "APPEND action failed with status {}\nexiting".format(
...
def upload_file(leader, group, auth_token, file_path):
    resp = lookups.upload_lookup_file(leader, auth_token, file_path, group)
    print(resp.json())
    if resp.status_code == 200:
        return resp.json()
    else:
        print(
            "PUT upload failed with returned status {}\nexiting!".format(r.status_code)
        )
        sys.exit(1)
...
def install_file(leader, group, auth_token, dest_file, uploaded_file):
    ...
    debug_log(
        "sending PATCH to {} with headers:{} and data:{}".format(
            url, headers, json.dumps(content)
        )
    )
...
def commit(leader, group, auth_token, file_path, msg):
    ...
    resp = versioning.create_commit(leader, auth_token, data)

    if resp.status_code == 200:
        return resp
    else:
        print(
            "create_commit failed with returned status {}\nexiting!".format(
                resp.status_code
            )
        )
        print(
            "details:\nurl: {}\nheaders: {}\ndata: {}\n".format(
                url, header, json.dumps(data)
            )
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if os.getenv("CRIBL_AUTH_TOKEN"):
        ...
        # for self-managed use the library's built-in method
            ...
            bearer_token = api_get_auth_data(
                args.leader, args.username, args.password
            ).json()["token"]
    ...
    commit_resp = commit(
        ...
        bearer_token,
        args.filepath,
        "automation: updated lookup file",
    )
    ...
    # deploy
    debug_log(
        "deploy the changes from commitID:{} to group:{}".format(commit_id, args.group)
    )
    deploy_results = groups.deploy_commit(
        base_url=args.leader + "/api/v1",
        cribl_auth_token=bearer_token,
        version=commit_id,
        worker_group=args.group,
    )
    ...
    if deploy_results.status_code == 200:
        ...
        print(
            "something happened at the deploy stage, status code: {}\n{}".format(
                deploy_results.status_code, deploy_results.text
            )
        )
